...
class ImageTextDatasetMERU(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img_path_pos = random.choice(glob.glob(os.path.join(self.work_dir, self.mode+"_folders", pos, "*")))
        img_path_neg = random.choice(glob.glob(os.path.join(self.work_dir, self.mode+"_folders", neg, "*")))
        pos = " ".join(pos.split("_")[1:][:random_index+1])
        neg = " ".join(neg.split("_")[1:][:random_index+1])
        img_pos = self.transform(Image.open(img_path_pos))
        img_neg = self.transform(Image.open(img_path_neg))
        return species_text, self.species_text.index(species_text), pos, neg, img_pos, img_neg
...
if __name__ == "__main__":
    ...
    dataset_meru = ImageTextDatasetMERU("bioclip/data/TreeOfLife-10M/dataset/evobio10m-dev/256x256", "train_folders/image_metadata.csv", mode="train")
