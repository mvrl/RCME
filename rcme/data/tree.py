...
def random_sample_by_rank(file_paths, query_text, rank_index):
    ...
    if rank_index != 0:
        valid_paths = [
            ...
            if path.split("_")[1:][rank_index] == query_text[rank_index]
            and path.split("_")[1:][rank_index - 1] == query_text[rank_index - 1]
        ]
    else:
        valid_paths = [
            path
    ...
    if rank_index != 0:
        valid_paths_negative = [
            path
            for path in file_paths
            if path.split("_")[1:][rank_index - 1] == query_text[rank_index - 1]
            and path.split("_")[1:][rank_index] != query_text[rank_index]
        ]
    else:
        valid_paths_negative = [
            path for path in file_paths if path.split("_")[1:][0] != query_text[0]
        ]
    if valid_paths_negative:
        random_path_negative = random.choice(valid_paths_negative)
    elif not valid_paths_negative:
        _, random_path_negative = random_sample_by_rank(
            file_paths, query_text, rank_index - 1
        )
...
if __name__ == "__main__":
    ...
    sampled_path = random_sample_by_rank(
        file_paths, target_rank_value.split(" "), rank_index
    )
