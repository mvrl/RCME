...
class RCME(pl.LightningModule):
    ...
    #         return ext_a
        ...
        GE_LOSS = 0
        for i in range(1, 6):
            GE_LOSS += torch.maximum(torch.zeros(P.shape[-1]).cuda(), ij2ext(i-1, i+1) - (torch.cos(P[i-1]).clip(0, 1)*torch.cos(P[i]).clip(0, 1)).acos()+np.pi/2).mean()
        GE_LOSS /= 5
    ...
    def shared_step(self, batch, train=True):
        ...
        loss = eloss + mloss + 0.1*loss_prior
